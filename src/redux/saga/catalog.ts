...
import {getCatalog, getItems, getLoadMoreItems} from "../../api/fetchApi.ts";
...
function* handleGetMoreItemsSaga({payload: {activatedCategory, offset}}: {
    payload: { activatedCategory: number, offset: number }
}): Generator {
    try {
        const data = yield getLoadMoreItems(activatedCategory, offset);
        yield put(catalogRequestSuccess(data));
    } catch (e: any) {
        yield put(catalogRequestFailure(e.message));
    }
...
function* handleGetItemsSaga({payload: {activatedCategory, searchQuery}}: {
    payload: { activatedCategory: number, searchQuery: string }
}): Generator {
    try {
        const data = yield getItems(searchQuery, activatedCategory);
        yield put(catalogRequestSuccess(data));
    } catch (e: any) {
        yield put(catalogRequestFailure(e.message));
    }
